...
export async function GET(request: NextRequest): Promise<NextResponse<AppointmentsListResponse | ErrorResponse>> {
  try {
    ...
    const url = `${BACKEND_API_URL}/appointments${queryString ? `?${queryString}` : ''}`;
    ...
    if (!response.ok) {
      ...
      return NextResponse.json(
        { message: errorText || 'Error fetching appointments' },
...
export async function POST(request: NextRequest): Promise<NextResponse<Appointment | ErrorResponse>> {
  try {
    ...
    const response = await fetch(`${BACKEND_API_URL}/appointments`, {
      method: 'POST',
      headers: {
    ...
    if (!response.ok) {
      const errorData = await response.json().catch(() => ({ message: 'Error creating appointment' }));
      return NextResponse.json(
        errorData,
        { status: response.status }
      );
